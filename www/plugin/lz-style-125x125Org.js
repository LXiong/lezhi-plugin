(function() {
    ...
    lz.render125x125 = function(result, isReload) {
        // for each lz-modules, find 125x125 styles
        for (var x = 0; x < lz.modules.nodes.length; x++) {
        	if (lz.modules.nodes[x].lezhiStyle == variables.STYLETYPE_125x125) {
	            var recommDiv = lz.modules.nodes[x], 
	                cssPrefix = recommDiv.idConfig ? "#" + recommDiv.id : "." + variables.STYLETYPE_125x125, // get id specific config
	                config = lz.modules.config[x], count = config.count, highlight = config.highlight,
	                pages, pageCounts = 2, currentPageNum, mType, feeds, feedLen, feedsList, insHtml, redirectUrl, p, i = 0, utilVar, found,
		                ...
		                for (currentPageNum = 0; currentPageNum < pages; currentPageNum++) {
		                	...
		                	if (currentPageNum == 0 && adEnabled) {
		                		if (adPreload) {
								    ...
								    w.mediav_ad_pub = variables.ADS_JS_IDS_MEDIAV["125x125"];
								    w.mediav_ad_width = "125";
								    w.mediav_ad_height= "125";
			                    ...
			                    for (var _lengths = (currentPageNum+1 - +adEnabled) * pageCounts; i < _lengths && i < feedLen && i < count; ++i) {
			                        ...
			                        if (curFeed.ad) {
			                        	// small ads are disabled in 125x125
	            ...
	            if (found) {
		            ...
		            recommDiv.styleObj = loadCss(x, cssPrefix, pages);
    ...
    function loadCss(index, cssPrefix, pages) {
        ...
        var config = lz.modules.config[index], fontSize = config.fontSize, bdcolor = config.bdcolor,
            ...
            cssStyles = cssPrefix + "{width:125px;height:125px;z-index:99999;overflow:hidden !important;margin:0 !important;padding:0 !important;}"
        ...
        cssStyles += cssPrefix + " .bShareRecommDiv3{overflow:hidden;position:relative;left:0;width:125px;border:0 solid " + bdcolor + ";"
            + "background:" + bgcolor + ";}";
        // css: .b-recomm-tab li
        cssStyles += cssPrefix + " .b-recomm-panel{width:" + 125 * pages + "px;display:none;margin:0 !important;overflow:hidden;padding:0 !important;float:left;position:relative;left:0;border:none;clear:both;}"
            // css: .b-recomm-panel.activated
            //+ ".b-recomm-panel.activated{display:block;border:1px solid " + bdcolor + ";}"
            // css: .b-recomm-list
            + cssPrefix + " .b-recomm-list{height:125px;width:125px;padding:0 !important;margin:0 !important;"
        ...
        // css: .b-recomm-list a
        cssStyles += cssPrefix + " .b-recomm-list a{padding:20px 0 !important;font-size:" + fontSize + "px;margin:0 !important;list-style:disc inside none !important;position:relative;"
        ...
        if (pages > 0) {
            cssStyles +=  cssPrefix + " .bShareRecommPages{text-align:center;position:absolute !important;width:125px;bottom:8px;}"
	...
	w.lezhi.render125x125(w.lezhi.variables.recommendation);
})();
