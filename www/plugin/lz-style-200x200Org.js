(function() {
    ...
    lz.render200x200 = function(result, isReload) {
        // for each lz-modules, find 200x200 styles
        for (var x = 0; x < lz.modules.nodes.length; x++) {
        	if (lz.modules.nodes[x].lezhiStyle == variables.STYLETYPE_200x200) {
	            var recommDiv = lz.modules.nodes[x], 
	                cssPrefix = recommDiv.idConfig ? "#" + recommDiv.id : "." + variables.STYLETYPE_200x200, // get id specific config
		                ...
		                for (currentPageNum = 0; currentPageNum < pages; currentPageNum++) {
		                	...
		                	if (currentPageNum == 0 && adEnabled) {
		                		if (adPreload) {
								    ...
								    w.mediav_ad_pub = variables.ADS_JS_IDS_MEDIAV["200x200"];
								    w.mediav_ad_width = "200";
								    w.mediav_ad_height= "200";
			                    ...
			                    for (var _lengths = (currentPageNum+1 - +adEnabled) * pageCounts; i < _lengths && i < feedLen && i < count; ++i) {
			                        ...
			                        if (curFeed.ad) {
			                        	// small ads are disabled in 200x200
    ...
    function loadCss(index, cssPrefix, pages) {
        ...
        var config = lz.modules.config[index], fontSize = config.fontSize, bdcolor = config.bdcolor,
            ...
            cssStyles = cssPrefix + "{width:200px;height:200px;z-index:99999;overflow:hidden !important;margin:0 !important;padding:0 !important;}"
        ...
        cssStyles += cssPrefix + " .bShareRecommDiv3{overflow:hidden;position:relative;left:0;width:200px;border:0 solid " + bdcolor + ";"
            + "background:" + bgcolor + ";}";
        // css: .b-recomm-tab li
        cssStyles += cssPrefix + " .b-recomm-panel{width:" + 200 * pages + "px;display:none;margin:0 !important;overflow:hidden;padding:0 !important;float:left;position:relative;left:0;border:none;clear:both;}"
            // css: .b-recomm-panel.activated
            //+ ".b-recomm-panel.activated{display:block;border:1px solid " + bdcolor + ";}"
            // css: .b-recomm-list
            + cssPrefix + " .b-recomm-list{height:200px;width:200px;padding:0 !important;margin:0 !important;"
        ...
        if (pages > 0) {
            cssStyles +=  cssPrefix + " .bShareRecommPages{text-align:center;position:absolute !important;width:200px;bottom:8px;}"
	...
	w.lezhi.render200x200(w.lezhi.variables.recommendation);
})();
