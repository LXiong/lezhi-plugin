(function() {
    ...
    lz.render250x250 = function(result, isReload) {
        // for each lz-modules, find 250x250 styles
        for (var x = 0; x < lz.modules.nodes.length; x++) {
        	if (lz.modules.nodes[x].lezhiStyle == variables.STYLETYPE_250x250) {
	            var recommDiv = lz.modules.nodes[x], feeds, feedLen, feedsList, insHtml, redirectUrl, p, i = 0,
	                config = lz.modules.config[x], cssPrefix = "." + variables.STYLETYPE_250x250, count = config.count,
		                ...
		                for (currentPageNum = 0; currentPageNum < pages; currentPageNum++) {
		                	...
		                	if (currentPageNum == 0 && adEnabled) {
		                		if (adPreload) {
								    ...
								    w.mediav_ad_pub = variables.ADS_JS_IDS_MEDIAV["250x250"];
								    w.mediav_ad_width = "250";
								    w.mediav_ad_height= "250";
			                    ...
			                    for (var _lengths = (currentPageNum+1 - +adEnabled) * pageCounts; i < _lengths && i < feedLen && i < count; ++i) {
			                        ...
			                        if (isAd) {
			                        	// small ads are disabled in 250x250
		    						...
		    						insHtml += '<img class="bshare-logos" src="' + p + '" style="width:100px !important;height:100px !important;"/>'
    ...
    function main(index, cssPrefix, pages) {
        ...
        var config = lz.modules.config[index], position = config.position, fontSize = config.fontSize, bdcolor = config.bdcolor,
            ...
            cssStyles = cssPrefix + "{width:250px;height:250px;z-index:99999;overflow:hidden !important;margin:0 !important;padding:0 !important;}"
        ...
        cssStyles += cssPrefix + " .bShareRecommDiv3{overflow:hidden;position:relative;left:0;width:250px;border:0px solid " + (bdcolor ? bdcolor : "#dadada") + ";"
            + (bgcolor ? "background:" + bgcolor + ";" : "") + "}";
        // css: .b-recomm-tab li
        cssStyles += cssPrefix + " .b-recomm-panel{width:" + 250 * pages + "px;display:none;margin:0 !important;overflow:hidden;padding:0 !important;float:left;position:relative;left:0;border:none;";
	    cssStyles += "clear:both;}"
            // css: .b-recomm-panel.activated
            //+ ".b-recomm-panel.activated{display:block;border:1px solid " + bdcolor + ";}"
            // css: .b-recomm-list
            + cssPrefix + " .b-recomm-list{height:250px;width:250px;padding:0 !important;margin:0 !important;"
        ...
        cssStyles += cssPrefix + " .b-recomm-list li{padding:0;display:block;text-align:center;text-indent:0 !important;"
            ...
            + "width:123px;height:248px;overflow:hidden;border:1px solid " + bgcolor + ";cursor:pointer;list-style:none !important;float:left;margin:0;}";
        ...
        if (pages > 0) {
            cssStyles +=  cssPrefix + " .bShareRecommPages{text-align:center;position:absolute !important;width:250px;bottom:8px;}"
	...
	w.lezhi.render250x250(w.lezhi.variables.recommendation);
})();
