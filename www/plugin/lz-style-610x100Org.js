(function() {
    ...
    lz.render610x100 = function(result) {
        // for each lz-modules, find 610x100 styles
        for (var x = 0; x < lz.modules.nodes.length; x++) {
        	if (lz.modules.nodes[x].lezhiStyle == variables.STYLETYPE_610x100) {
	            var recommDiv = lz.modules.nodes[x], 
	                cssPrefix = recommDiv.idConfig ? "#" + recommDiv.id : "." + variables.STYLETYPE_610x100, // get id specific config
		                ...
		                for (currentPageNum = 0; currentPageNum < pages; currentPageNum++) {
		                	...
		                	if (currentPageNum == 0 && adEnabled) {
			                    ...
			                    for (var _lengths = (currentPageNum+1 - +adEnabled) * pageCounts; i < _lengths && i < feedLen && i < count; ++i) {
			                        ...
			                        if (curFeed.ad) {
			                        	// small ads are disabled in 610x100
		                            ...
		                            insHtml += '" hidefocus="true" style="padding:20px 0 !important;">';
    ...
    function loadCss(index, cssPrefix, pages) {
        ...
        var config = lz.modules.config[index], fontSize = config.fontSize, bdcolor = config.bdcolor,
            ...
            cssStyles = cssPrefix + "{width:610px;height:100px;z-index:99999;overflow:hidden !important;margin:0 !important;padding:0 !important;}"
        ...
        cssStyles += cssPrefix + " .bShareRecommDiv3{overflow:hidden;position:relative;left:0;width:610px;border:0 solid " + bdcolor + ";"
            + "background:" + bgcolor + ";}";
        // css: .b-recomm-tab li
        cssStyles += cssPrefix + " .b-recomm-panel{width:" + 610 * pages + "px;display:none;margin:0 !important;overflow:hidden;padding:0 !important;float:left;position:relative;left:0;border:none;clear:both;}"
            // css: .b-recomm-panel.activated
            //+ ".b-recomm-panel.activated{display:block;border:1px solid " + bdcolor + ";}"
            // css: .b-recomm-list
            + cssPrefix + " .b-recomm-list{height:100px;width:610px;padding:0 !important;margin:0 !important;"
        ...
        if (pages > 0) {
            cssStyles +=  cssPrefix + " .bShareRecommPages{text-align:center;position:absolute !important;width:610px;bottom:8px;}"
	...
	w.lezhi.render610x100(w.lezhi.variables.recommendation);
})();
